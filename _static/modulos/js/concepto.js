...
function form_edit(id){
	...
	$.get(url+controlador+'/ajax_edit/'+id, function(data, textStatus, xhr) {
		if(textStatus=="success"){
			$('[name="id"]').val(data[0].idconcepto);
			$('[name="idtipomovimiento"]').val(data[0].tipomovimiento_idtipomovimiento);
...
function save(){
	labels = ['idtipomovimiento','descripcion'];
	fallas = false;
