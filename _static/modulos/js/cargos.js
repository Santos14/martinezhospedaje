...
function init(){
	$.get(url+"cargos/tableList", function(data) {
...
function form_add(){
	save_method = 'add';
	$("#title_form").text("Agregar Cargos");
	$("#btn_save").text("Crear");
	$('#form')[0].reset();
	$("#id").val("");
...
function form_edit(id){
	$("#title_form").text("Editar Cargos");
	$("#btn_save").text("Actualizar");
	$('#form')[0].reset();
	$.get(url+'cargos/ajax_edit/'+id, function(data, textStatus, xhr) {
		if(textStatus=="success"){
			$('[name="id"]').val(data[0].idcargo);
